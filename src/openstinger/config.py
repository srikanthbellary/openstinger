...
class HarnessConfig(BaseModel):
    ...
    def resolved_profile_dirs(self, root: Path | None = None) -> list[Path] | None:
        if self.ingestion.profile_dirs is None:
            return None
        base = root or self._root_dir or Path.cwd()
        return [resolve_path(d, base) for d in self.ingestion.profile_dirs if d] # type: ignore
