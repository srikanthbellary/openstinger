...
python -m openstinger.gradient.mcp.server       # all 32 tools on port 8766
```
...
To give the agent access to OpenStinger's 32 tools (so it can call `memory_query`, `ops_status`, etc.), add the MCP server to Qwen-Agent's `function_list`:
